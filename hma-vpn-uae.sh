...
cat servers.txt | grep -i -e "$grep" | grep -i -e "$proto" > /tmp/hma-servers
...
cat vpn.cfg | sed 's/auth-user-pass/auth-user-pass \/home\/james\/.ssh\/hma.auth/g' > /tmp/hma-config.cfg 2>/dev/null
